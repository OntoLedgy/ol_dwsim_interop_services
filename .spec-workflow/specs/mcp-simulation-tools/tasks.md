...
- [x] 1. Create SimulationResultDto in C#
  ...
  - Purpose: Provide structured result format for pythonnet bridge
  - _Leverage: `DwsimWorker/Contracts/CapeOpen/MaterialStreamDto.cs`, `DwsimWorker/Models/CalculationResult.cs`_
...
- [x] 2. Enhance CalculationAdapter with status and caching methods
  ...
  - Update `RunCalculation` to cache results in FlowsheetContext
  - Purpose: Support get_status and get_results tools with cached data
  - _Leverage: Existing `CalculationAdapter.cs`, `FlowsheetContext.cs`_
...
- [x] 3. Create Python request models for simulation tools
  ...
  - GetStatusRequest: session_id (required)
  - GetResultsRequest: session_id (required), object_id (optional)
  - Purpose: Provide type-safe input validation for MCP tools
  - _Leverage: `models/requests/create_session_request.py`, `models/requests/add_stream_request.py`_
...
- [x] 4. Create Python response models for simulation tools
  ...
  - SimulationStatusResponse: status, is_running, last_run_timestamp, elapsed_ms
  - StreamPropertiesResponse: id, name, temperature_k, pressure_pa, total_molar_flow_mol_per_s, phases
  - Purpose: Provide structured, validated output format for MCP tools
  - _Leverage: `models/responses/create_session_response.py`, `DwsimWorker/Contracts/CapeOpen/MaterialStreamDto.cs`_
...
- [x] 5. Create simulation error model
  ...
  - Include diagnostic fields: solver_messages, failed_unit_id, convergence_state
  - Purpose: Provide structured error information for simulation failures
  - _Leverage: `models/errors/session_error.py`, `models/errors/resource_limit_error.py`_
  - _Requirements: 1, 4_
...
- [x] 6. Implement simulation.py MCP tools
  ...
  - Implement handlers: `_handle_run()`, `_handle_get_status()`, `_handle_get_results()`
  - Integrate with OperationTimeoutRunner for timeout enforcement
  - Integrate with ResourceLimitGuard for memory monitoring
  - Purpose: Provide LLM-facing MCP tools for simulation execution
  - _Leverage: `tools/session.py`, `tools/flowsheet.py`, `ipc/session_client.py`, `limits/operation_timeout_runner.py`_
...
- [x] 7. Extend session_client with simulation methods
  ...
  - Add `get_calculation_results(session_id, object_id=None)` method
  - Convert C# DTOs to Python dicts using pythonnet_dto_converter
  - Purpose: Provide Python-side bridge to C# CalculationAdapter
  - _Leverage: Existing `session_client.py` methods, `converters/pythonnet_dto_converter.py`, `ipc/clr_loader.py`_
  - _Requirements: 1, 2, 3, 5_
...
- [x] 8. Register simulation tools in MCP server
  ...
  - Purpose: Make simulation tools available to MCP clients
  - _Leverage: Existing tool registration in `server.py`, `tools/registry.py`_
  - _Requirements: 1, 2, 3_
...
- [x] 9. Create C# unit tests for SimulationResultDto
  ...
  - Test integration with CalculationResult conversion
  - Purpose: Ensure DTO reliability and serialization correctness
  - _Leverage: `DwsimWorker.Tests/Contracts/MaterialStreamDtoTests.cs` pattern_
  - _Requirements: 1, 3, 5_
...
- [x] 10. Create C# unit tests for CalculationAdapter enhancements
  ...
  - Purpose: Ensure adapter enhancements work correctly
  - _Leverage: Existing tests in `CalculationAdapterTests.cs`_
  - _Requirements: 2, 3, 5_
...
- [x] 11. Create Python unit tests for simulation tools
  ...
  - Test tool dispatching to correct handlers
  - Mock session_client to test Python layer in isolation
  - Test error handling for all error scenarios
  - Purpose: Ensure Python tool implementation is reliable
  - _Leverage: `tests/unit/tools/test_session.py` pattern, `tests/conftest.py` fixtures_
  - _Requirements: 1, 2, 3, 4_
...
- [x] 12. Create Python-C# integration tests
  ...
  - Test timeout scenario with long-running calculation
  - Test convergence failure scenario with invalid flowsheet
  - Test result caching (multiple get_results calls)
  - Purpose: Validate end-to-end Python-C# integration
  - _Leverage: `tests/integration/test_session_integration.py`, `tests/integration/conftest.py`_
...
- [x] 13. Update MCP server documentation
  ...
  - Purpose: Provide clear documentation for tool users
