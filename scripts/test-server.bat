...
curl.exe -s -o nul -w "HTTP Status: %%{http_code}" %SERVER_URL%/mcp/ > "%TEMP%\http_status.txt"
...
echo [2/4] Testing MCP initialize (with trailing slash)...
curl.exe -s -X POST %MCP_ENDPOINT% -H "Content-Type: application/json" -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"protocolVersion\":\"2024-11-05\",\"capabilities\":{},\"clientInfo\":{\"name\":\"test\",\"version\":\"1.0\"}}}" > "%TEMP%\mcp_test.json"
echo Response:
...
if %ERRORLEVEL% NEQ 0 (
    echo.
    echo [WARN] MCP initialize with trailing slash failed, trying without...
...
curl.exe -s -X POST %MCP_ENDPOINT% -H "Content-Type: application/json" -d "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/list\",\"params\":{}}" > "%TEMP%\mcp_tools.json"
echo Response:
...
curl.exe -v -X POST %MCP_ENDPOINT% -H "Content-Type: application/json" -d "{\"jsonrpc\":\"2.0\",\"id\":3,\"method\":\"ping\",\"params\":{}}" 2>&1
echo.
